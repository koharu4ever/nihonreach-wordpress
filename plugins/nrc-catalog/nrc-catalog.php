...
function nrc_register() {
	register_post_type( 'nrc_product', array(
		'labels' => array( 'name' => '产品', 'singular_name' => '产品', 'add_new_item' => '新增产品', 'edit_item' => '编辑产品', 'all_items' => '所有产品' ),
		'public' => true, 'has_archive' => 'products', 'rewrite' => array( 'slug' => 'product' ),
		'menu_icon' => 'dashicons-products', 'supports' => array( 'title', 'editor', 'thumbnail', 'excerpt', 'revisions', 'author' ),
		'capability_type' => 'post', 'map_meta_cap' => true, 'show_in_rest' => false,
	) );
	register_taxonomy( 'nrc_category', 'nrc_product', array(
		'labels' => array( 'name' => '产品分类', 'singular_name' => '产品分类' ),
		'public' => true, 'hierarchical' => true, 'show_admin_column' => true,
		'rewrite' => array( 'slug' => 'product-category' ),
	) );
	register_post_meta( 'nrc_product', '_nrc_model', array(
...
add_filter( 'use_block_editor_for_post_type', function ( $use, $type ) { return 'nrc_product' === $type ? false : $use; }, 10, 2 );
...
add_action( 'add_meta_boxes_nrc_product', function () { add_meta_box( 'nrc-model', '产品型号', 'nrc_model_box', 'nrc_product', 'normal', 'high' ); } );

/** Validate a submitted model without changing content. Missing and empty are distinct. */
function nrc_save_model( $post_id ) {
	if ( 'nrc_product' !== get_post_type( $post_id ) || wp_is_post_revision( $post_id ) || wp_is_post_autosave( $post_id ) || ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) ) { return; }
	if ( ! current_user_can( 'edit_post', $post_id ) || ! isset( $_POST['nrc_model_nonce'] ) || ! is_string( $_POST['nrc_model_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nrc_model_nonce'] ) ), 'nrc_model_' . $post_id ) ) { return; }
	if ( ! array_key_exists( 'nrc_model', $_POST ) ) { return; }
	...
	if ( '' === $value ) { delete_post_meta( $post_id, '_nrc_model' ); } else { update_post_meta( $post_id, '_nrc_model', $value ); }
}
...
function nrc_model_notice() {
	...
	if ( ! $screen || 'nrc_product' !== $screen->post_type || ! current_user_can( 'edit_post', $id ) ) { return; }
...
function nrc_public_query( $query ) {
	...
	if ( $query->is_search() || $query->is_feed() ) { $query->set( 'has_password', false ); }
}
add_action( 'pre_get_posts', 'nrc_public_query' );
register_activation_hook( __FILE__, function () { nrc_register(); flush_rewrite_rules(); } );
register_deactivation_hook( __FILE__, function () { unregister_post_type( 'nrc_product' ); unregister_taxonomy( 'nrc_category' ); flush_rewrite_rules(); } );
