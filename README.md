...
当前状态：仓库已公开，维护者已通过 Coolify 部署到独立 VPS 容器环境。

- [访问线上演示](https://wp.kral-koharu.com/)
- [浏览产品目录](https://wp.kral-koharu.com/products/)
- [生产验收记录](docs/production-acceptance.md)：2026-09-05 实际检查 HTTPS、页面、图片和管理员登录。

这是个人求职演示作品，不代表真实客户项目、商业交易或线上用户规模。后台仅供维护者使用，不提供公开管理员账号。
...
核心为单篇页面输出 canonical；归档和归档分页没有额外添加 canonical，不把第二页错误指向第一页。初始化设置建议搜索引擎不抓取，不宣称完整 SEO 优化。无 WooCommerce、订单、支付、会员、复杂表单、Headless 或 Laravel 同步。

与 Laravel NihonReach 只共享虚构场景和获准复用的 Demo 图片；目录、账号、数据库、网络和文件卷独立。本地项目共享 Docker、内存和磁盘；线上项目共享 VPS 资源，均不等于两台虚拟机。初次本地验收时未提交或部署；后续已推送公开仓库并完成 Coolify 部署，没有修改旧 Laravel 项目。
